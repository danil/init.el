(defgroup init-emacs-lisp-mode nil
  "init-emacs-lisp-mode"
  :group  'editing
  :tag    "init-emacs-lisp-mode"
  :prefix "init-emacs-lisp-mode-"
  :link   '(url-link :tag "GitHub" "https://github.com/danil/init.el"))

(defcustom init-emacs-lisp-mode-patterns '()
  "Regexp patterns associated with `emacs-lisp-mode'."
  :group 'init-emacs-lisp-mode)

(defcustom init-emacs-lisp-mode-map-prefix "C-v " "init-emacs-lisp-mode" :group 'init-emacs-lisp-mode)

(defcustom init-emacs-lisp-mode-rainbow-identifiers-stop-words
  '(
    ;; Emacs standard functions.
    "1value"
    "2C-associate-buffer"
    "2C-command"
    "2C-split"
  ...
  "Do not highlight in `emacs-lisp-mode'."
  :group 'init-emacs-lisp-mode)

(defun init-emacs-lisp-mode ()
  (dolist (p init-emacs-lisp-mode-patterns)
    (add-to-list 'auto-mode-alist (cons p 'emacs-lisp-mode)))

  (if (boundp 'emacs-lisp-mode-map) (noxrcp-emacs-lisp-mode--init)
    (with-eval-after-load 'elisp-mode (noxrcp-emacs-lisp-mode--init))))

(defun noxrcp-emacs-lisp-mode--init ()
  (let ((prfx init-emacs-lisp-mode-map-prefix)
        (map emacs-lisp-mode-map))
    (define-key map (kbd "C-c C-f n") 'beginning-of-defun)
    (define-key map (kbd (concat prfx " ! b")) 'init-emacs-lisp-mode-eval-buffer)
    (define-key map (kbd (concat prfx " ! r")) 'init-emacs-lisp-mode-eval-region)
    (define-key map [?\C-\M-i] nil)))

(defun init-emacs-lisp-mode-eval-buffer ()
  (interactive)
  (message (format "Eval: %s" (buffer-name)))
  (eval-buffer))

(defun init-emacs-lisp-mode-eval-region (start end)
  (interactive "r")
  (message "Eval region")
...
(defun init-emacs-lisp-mode-rainbow-identifiers-setup ()
  (when (equal major-mode 'emacs-lisp-mode)
    ...
    (add-hook 'rainbow-identifiers-filter-functions
              'init-emacs-lisp-mode--rainbow-identifiers-filter)

    (init-rainbow-identifiers--lazy-setup)))

(defun init-emacs-lisp-mode--rainbow-identifiers-filter (beg end)
  ...
  (let ((face-cur (or (get-char-property beg 'read-face-name)
    ...
    (and
     ...
     (not (member str-cur init-emacs-lisp-mode-rainbow-identifiers-stop-words)))))
